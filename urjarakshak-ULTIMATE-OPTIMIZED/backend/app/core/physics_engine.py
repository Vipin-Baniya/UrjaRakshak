...
class PhysicsEngine:
    ...
    def validate_energy_conservation(
        ...
        if output_energy_mwh > input_energy_mwh * 1.1:
            return self._create_refused_result(
                "Output > Input violates conservation of energy",
                input_energy_mwh,
                output_energy_mwh
            )
    ...
    def _compute_line_loss(
        ...
        P_loss = I² × R × t
        where R depends on material, length, temperature
        """
        if line.resistance_ohms and line.length_km:
            # Detailed calculation
            # R(T) = R(T₀) × [1 + α(T - T₀)]
            temp_coeff = 0.00393  # for copper
            R = line.resistance_ohms * (1 + temp_coeff * (self.temperature - 20))
            
            # Estimate current from power
            if line.voltage_kv:
                I = (energy_mwh * 1000) / (line.voltage_kv * np.sqrt(3))  # 3-phase
                loss_mwh = (I ** 2) * R * 1e-6  # Convert W to MWh
            else:
                # Simplified: 2% for distribution, 1% for transmission
                loss_mwh = energy_mwh * 0.02
            
            method = "i2r_physics_calculation"
        else:
            # Empirical estimation
            if "transmission" in line.component_type.lower():
                loss_mwh = energy_mwh * 0.015  # 1.5%
            else:
                loss_mwh = energy_mwh * 0.025  # 2.5%
